...
class Info(commands.Cog):
    ...
    @cog_ext.cog_slash(name="population", description="Displays how many members the guild has", guild_ids=settings.guilds)
    ...
    @cog_ext.cog_slash(name="info", guild_ids=settings.guilds, description="General Information about the Jet Project")
    ...
    @cog_ext.cog_slash(name="difficulty", guild_ids=settings.guilds, description="Make the game harder")
    ...
    @cog_ext.cog_slash(name="editor", guild_ids=settings.guilds, description="Skip a quest, get money, skills, etc")
    async def editor(self, ctx: SlashContext):
        """Skip a quest, get money, skills, etc"""
        await ctx.send(embed=messages.editor(ctx))
    # all in one

    @cog_ext.cog_slash(name="hideout", guild_ids=settings.guilds, description="How to shorten hideout craftings, longer raids,no weight, etc.")
    ...
    @cog_ext.cog_slash(name="server", guild_ids=settings.guilds, description="Fix for Server Closes Instantly")
    async def server(self, ctx: SlashContext):
        """Server Closes Instantly"""
        await ctx.send(embed=messages.server(ctx))
# general install

    @cog_ext.cog_slash(name="install", guild_ids=settings.guilds, description="How to install Jet 12.9")
...
# send noobs to kovac's discord
    ...
    @cog_ext.cog_slash(name="kovacs", guild_ids=settings.guilds, description="Kovacs related help")
    async def kovacs(self, ctx: SlashContext):
        """Kovacs related help"""
        await ctx.send(embed=messages.kovacs(ctx))

    @cog_ext.cog_slash(name="key", guild_ids=settings.guilds, description="A key has already been added fix")
