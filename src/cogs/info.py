...
footerButtons = [
    ...
    create_button(
        style=5,
        label="Invite Me!",
        url="https://discord.com/api/oauth2/authorize?client_id=752568871986397216&permissions=2213932102&scope=bot%20applications.commands",
        emoji="♥",
    ),
]
...
class info(commands.Cog):
    ...
    @cog_ext.cog_slash(
        name="editor",
        description="Skip a quest, get money, skills, etc",
    ...
    async def editor(self, ctx):
        ...
        await ctx.send(
            embed=messages.editor(), components=[create_actionrow(*buttons), footerBar]
        )

    @cog_ext.cog_slash(
        name="info",
        description="General Information about the Jet Project",
        permissions=blacklist,
    )
    async def info(self, ctx):
        buttons = [
            create_button(
                style=5, label="Documentation", url="https://docs.justemutarkov.eu/"
            ),
            create_button(
                style=5, label="Mods Archive", url="https://justemutarkov.eu/download"
            ),
            create_button(
                style=5, label="Jet Discord Invite", url="https://discord.gg/Gbn5bTV"
            ),
        ]

        await ctx.send(
            embed=messages.info(), components=[create_actionrow(*buttons), footerBar]
        )

    @cog_ext.cog_slash(
        name="port",
        description="Solutions for port already in use error",
        permissions=blacklist,
    )
