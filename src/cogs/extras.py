...
class extras(commands.Cog):
    ...
    @cog_ext.cog_slash(
        name="bonk",
        description="bonk another member",
        options=[
            create_option(
                name="member",
                description="Tag Member to bonk",
                option_type=6,
                required=True,
            )
        ],
    ...
    @cog_ext.cog_slash(
        name="funfix",
        description="General Form to fix any issues related to the FunServer",
        guild_ids=funserver,
        options=[
            create_option(
                name="ingame",
                description="your character name inside the fun server",
                option_type=3,
                required=True,
            ),
            create_option(
                name="quest",
                description='name or id of the quest that you need fixed if "key already added error" type duplicate item',
                option_type=3,
                required=True,
            ),
        ],
    ...
    async def funfix(self, ctx: SlashContext, ingame: str, quest: str):
        msg = discord.Embed(
            title=f" FunServer Fix: {ctx.author}",
            description=f"Ingame Name: {ingame} \n Quest Id or Name: {quest}",
        ...
        await ctx.send(
            f"{ctx.author.mention} Successfully notified the Fun Server team",
            hidden=True,
        )
