...
def key():
    s1 = discord.Embed(title="Step 1", description="Locate your Character.json (Server folder/user/profiles/AID/here)",
                       color=warmup.rmd(), timestamp=datetime.now())
    s2 = discord.Embed(title="Step 2", description="Head over to <#829464850895077406> or <#740002608424550522> and do !!clean with the profile attached",
                       color=warmup.rmd(), timestamp=datetime.now())
    s3 = discord.Embed(
        title="Step 3", description="Go back to where you grabbed the character.json and replace it with the output of the bot")
    s4 = discord.Embed(
    ...
    s5 = discord.Embed(title="Step 5", description="Go to your server folder and extract the file you just downloaded into the SRC folder, when asked select replace files",
                       color=warmup.rmd(), timestamp=datetime.now())
